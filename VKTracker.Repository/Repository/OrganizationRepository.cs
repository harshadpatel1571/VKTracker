...
namespace VKTracker.Repository.Repository
{
    ...
    {
        public async Task<DataTableResponseCarrier<OrganizationViewModel>> GetList(DataTableFilterViewModel filterDto, int userId, int organizationId)
        {
            var db = new VKTrackerEntities();

            try
            {
                var result = db.Organizations.Where(x => x.IsActive && (organizationId == 0 ? true : x.OrganizationId == organizationId)).AsNoTracking().AsQueryable();
        ...
        public async Task<bool> Save(OrganizationViewModel objModel, int userId, int organizationId)
        {
            var db = new VKTrackerEntities();
            try
            {
