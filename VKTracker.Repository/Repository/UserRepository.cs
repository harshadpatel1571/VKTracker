...
namespace VKTracker.Repository.Repository
{
    ...
    {
        public async Task<DataTableResponseCarrier<UserViewModel>> GetList(DataTableFilterViewModel filterDto, int userId, int organizationId)
        {
            var db = new VKTrackerEntities();

            try
            {
                var result = db.Users.Where(x => x.IsActive && (organizationId == 0 ? true : x.OrganizationId == organizationId)).AsNoTracking().AsQueryable();
        ...
        public async Task<bool> Save(UserViewModel objModel, int userId, int organizationId)
        {
            var db = new VKTrackerEntities();
            try
            {
        ...
        public async Task<bool> GetDuplicate(int id, string name, int organizationId)
        {
            ...
            try
            {
                return await db.Users.AnyAsync(x => x.Id != id && x.OrganizationId == organizationId && x.UserName.ToLower() == name.ToLower()).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public async Task<List<BindDropdownViewModel>> BindUserOrganizationDDl(int userId)
        {
            var db = new VKTrackerEntities();
            try
            {
                return await db.UserOrganizations.Where(x => x.UserId == userId && x.Organization.IsActive).Select(x => new BindDropdownViewModel
                {
