...
namespace VKTracker.Repository.Repository
{
    ...
    {
        public async Task<DataTableResponseCarrier<LocationViewModel>> GetList(DataTableFilterViewModel filterDto, int userId, int organizationId)
        {
            var db = new VKTrackerEntities();

            try
            {
                var result = db.Locations.Where(x => x.IsActive && x.OrganizationId == organizationId).AsNoTracking().AsQueryable();
        ...
        public async Task<bool> Save(LocationViewModel objModel, int userId, int organizationId)
        {
            var db = new VKTrackerEntities();
            try
            {
        ...
        public async Task<bool> GetDuplicate(int id, string locationName, int organizationId)
        {
            var db = new VKTrackerEntities();
            try
            {
                return await db.Locations.AnyAsync(x => x.Id != id && x.LocationName.ToLower() == locationName.ToLower() && x.IsActive == true && x.OrganizationId == organizationId).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public async Task<List<BindDropdownViewModel>> BindLocationDDl(int organizationId)
        {
            var db = new VKTrackerEntities();
            try
            {
                return await db.Locations.Where(x => x.IsActive && x.OrganizationId == organizationId).Select(x => new BindDropdownViewModel
                {
                    Id = x.Id,
                    Name = x.LocationName
                }).ToListAsync().ConfigureAwait(false);
