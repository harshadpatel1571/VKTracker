function bindParcelCode() {
    ...
    }).buttons().container().appendTo('#parcelCodeHeader');
}
...
function bindParcelCodeLogGrid(id) {
    $('#gridParcelCodeLog').DataTable().destroy();
    $('#gridParcelCodeLog').DataTable({
        paging: false,
        lengthChange: false,
        searching: false,
        ordering: true,
        ...
        filter: true,
        ajax: {
            url: "/Master/GetParcelCodeLogList/" + id,
            type: "POST",
            datatype: "json"
        },
        columns: [
    ...
    }).buttons().container().appendTo('#gridParcelCodeLog_wrapper .col-md-6:eq(0)');
}